		...
		<div class="w-100 buscador-filtro-wrap-fieldset">
			...
			<fieldset>
				...
				<div class="drop-down no-display" id="drop-down-modelo">
					<ul>
						<li>
							<span class="drop-li-marca" field="bf-modelo" value="0">
								Sin modelo
							</span>
						</li>
					</ul>
...
<script type="text/javascript">
	...
	$(function(){
		...
		//	console.log(select_target);
			...
			if ($(this).attr('class') == 'li-ano') {
			//	console.log('Li ano');
				busca = 'versiones';
				desde = 'anos';
			}

			if ($(this).attr('class') == 'li-version') {
				console.log('Li version');
				busca = '';
				desde = '';
			...
			$.ajax({
				...
				success: function(response){
					...
					if (busca == 'modelos') {
						...
						$('#bf-modelo').parent().find('.bf-select').html('Modelo <i class="fa fa-chevron-down"></i>');
					...
					if (busca == 'anos') {
						...
						drop_down = $('#drop-down-ano').find('ul');

						ul_content = '';

						$.each(response['anos'], function(key, value){
							ul_content += '<li class="li-ano"><span class="drop-li-marca" field="bf-ano" value="'+value["id"]+'" style="text-transform:capitalize;">'+value["nombre"]+'</span></li>';
					...
					if (busca == 'versiones') {
						...
						drop_down = $('#drop-down-version').find('ul');

						ul_content = '';

						$.each(response['versiones'], function(key, value){
							ul_content += '<li class="li-version"><span class="drop-li-marca" field="bf-version" value="'+value["id"]+'" style="text-transform:capitalize;">'+value["nombre"]+'</span></li>';
